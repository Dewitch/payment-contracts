...
async function main() {
  ...
  const SubscriptionHandler = await hre.ethers.getContractFactory(
    "SubscriptionHandler"
  );
  //deploy the money router account using the proper host address and the address of the first signer
  const subscriptionHandler = await SubscriptionHandler.deploy(
    sf.settings.config.hostAddress,
    signers[0].address
  );

  await subscriptionHandler.deployed();

  console.log("SubscriptionHandler deployed to:", subscriptionHandler.address);
}
